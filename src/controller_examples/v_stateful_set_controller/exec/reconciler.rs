    ...
    // ensures (res.0@, opt_request_to_view(&res.1)) == model_reconciler::reconcile_core(v_stateful_set@, opt_response_to_view(&resp_o), state@),
{
    ...
    //         let statefuls = v_stateful_set.spec().statefuls().unwrap_or(0);
    //         if statefuls < 0 {
    //             return (error_state(state), None);
    //         }
    //         let desired_statefuls: usize = statefuls as usize;
    //         if filtered_pods.len() == desired_statefuls {
    //             let state_prime = VStatefulSetReconcileState {
    //                 reconcile_step: VStatefulSetReconcileStep::Done,
    //                 ..state
    //             };
    //             return (state_prime, None);
    //         } else if filtered_pods.len() < desired_statefuls {
    //             let diff =  desired_statefuls - filtered_pods.len();
    //             let pod = make_pod(v_stateful_set);
    ...
    //             });
    //             let state_prime = VStatefulSetReconcileState {
    //                 reconcile_step: VStatefulSetReconcileStep::AfterCreatePod(diff - 1),
    //                 ..state
    //             };
    //             return (state_prime, Some(Request::KRequest(req)));
    //         } else {
    //             let diff = filtered_pods.len() - desired_statefuls;
    ...
    //             });
    //             let state_prime = VStatefulSetReconcileState {
    //                 reconcile_step: VStatefulSetReconcileStep::AfterDeletePod(diff - 1),
    ...
    //         }
    //     },
    //     VStatefulSetReconcileStep::AfterCreatePod(diff) => {
    //         let diff = *diff;
    //         if !(resp_o.is_some() && resp_o.as_ref().unwrap().is_k_response()
    ...
    //         }
    //     },
    //     VStatefulSetReconcileStep::AfterDeletePod(diff) => {
    //         let diff = *diff;
    //         if !(resp_o.is_some() && resp_o.as_ref().unwrap().is_k_response()
...
// fn make_pod(v_stateful_set: &VStatefulSet) -> (pod: Pod)
//     requires v_stateful_set@.well_formed(),
// {
//     let template = v_stateful_set.spec().template().unwrap();
...
//         }
//         metadata.set_generate_name(v_stateful_set.metadata().name().unwrap().concat(new_strlit("-")));
